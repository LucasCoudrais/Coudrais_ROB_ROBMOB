...
class ShortPathMng(Node):
    ...
    def inflate_map(self, map, map_resolution):
        ...
        for index in range(0, size):
            ...
            radius = math.ceil(self.inflate_radius / map_resolution)

            if map[current_index_y][current_index_x] == self.MAP_OBSTACLE_VALUE:
                i = 0
                while i < radius:
                    j = 0
                    while j < radius:
                        if current_index_y + j < self.map_height and current_index_x + i < self.map_width:
                            new_inflated_map[current_index_y + j][current_index_x + i] = self.MAP_OBSTACLE_VALUE
                        if current_index_y - j >= 0 and current_index_x - i >= 0:
                            new_inflated_map[current_index_y - j][current_index_x - i] = self.MAP_OBSTACLE_VALUE
                        if current_index_y - j >= 0  and current_index_x + i < self.map_width:
                            new_inflated_map[current_index_y - j][current_index_x + i] = self.MAP_OBSTACLE_VALUE
                        if current_index_y + j < self.map_height and current_index_x - i >= 0:
                            new_inflated_map[current_index_y + j][current_index_x - i] = self.MAP_OBSTACLE_VALUE
                        j += 1
                    i += 1
