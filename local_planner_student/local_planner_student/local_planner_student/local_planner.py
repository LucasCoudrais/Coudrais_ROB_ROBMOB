...
class LocalPlanner(Node):
    ...
    def scan_callback(self, scan):
        ...
        self.isObstacle = False
        for element in scan.ranges :
            if element < self.Obstacle_range and element != 0.0 : 
                self.isObstacle = True
        if self.isObstacle :
            self.get_logger().info("Warning Obstacle")
    ...
    def compute_dist_angle(self):
        ...
        if len(self.pathPoses) > 0:
            ...
            distCurTarget = sqrt((self.curPose2D.x - self.pathPoses[0].pose.position.x)**2 + (self.curPose2D.y - self.pathPoses[0].pose.position.y)**2)
            steering = atan2(  (self.pathPoses[0].pose.position.y-self.curPose2D.y) , (self.pathPoses[0].pose.position.x-self.curPose2D.x) )

            angle = self.shortest_angle_diff(steering, self.curPose2D.theta) 
            # quaternion = [self.pathPoses[0].pose.orientation.x, self.pathPoses[0].pose.orientation.y, self.pathPoses[0].pose.orientation.z, self.pathPoses[0].pose.orientation.w]
            # roll, pitch, yaw = euler_from_quaternion(quaternion)
            # angle = self.shortest_angle_diff(self.curPose2D.theta, yaw)
            
            # self.get_logger().info(str(self.pathPoses[0].pose.position.x))
            # self.get_logger().info(str(self.pathPoses[0].pose.position.y))
    ...
    def compute_velocity(self, dist, angle, goalState):
        ...
        twist.angular.z = min(angle * self.K_angular, self.Sat_angular)  #TODO for students : Change 0 with gain and saturation (both ROSPARAM) applied to angle (as already done for linear velocity)

        if fabs(angle) < self.Angle_to_allow_linear:
            if goalState == "Reach in progress" or goalState == "New Goal":  #TODO for students : modify string matching with the state (help in pathSequencer docstring)  
                if self.isObstacle:
                    twist.linear.x = 0.0
                else:
    ...
    def local_planning(self):
        ...
        elif goalState == "Last Goal pose (position + orientation) Reached" or goalState == "Last Goal position Reached":    #TODO for students : modify string matching with the state (help in pathSequencer docstring)        
